...
qflx_tot_pf,qflx_grnd_pf,qflx_soi_pf,qflx_eveg_pf,qflx_tveg_pf,qflx_in_pf,swe_pf,surfalb_pf,t_g_pf,   &
...
clm_last_rst,clm_daily_rst,rz_water_stress_typepf, pf_nlevsoi, pf_nlevlak)
  ...
  real(r8) :: surfalb_pf((nx+2)*(ny+2)*3)        ! ARY : surface albedo output var to send to ParFlow, on grid w/ ghost nodes for current proc but nz=1 (2D)
...
!
  ...
  do t=1,drv%nch
     ...
     if (clm(t)%planar_mask==1) then
        ...
        surfalb_pf(l)      = clm(t)%surfalb !ARY        

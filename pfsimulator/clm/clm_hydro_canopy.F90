...
subroutine clm_hydro_canopy (clm)
  ...
  ! Parameters for saturation vapor pressure calculation @AXRY after °RMM2025
  ...
  ! Variables for wetbulb rain-snow partitioning @AXRY after @RMM 2025
  ...
  !     @AXRY/@RMM 2025: Only use itypprc shortcut for CLM default method (type 0).
  ...
  if (clm%snl == 0 .AND. clm%qflx_snow_grnd > 0.0 .AND. clm%snowdp >= 0.01) then  
     ...
     clm%snowage_vis = 0.                         ! VIS band snow age @AXRY/@RMM 2025
     clm%snowage_nir = 0.                         ! NIR band snow age @AXRY/@RMM 2025
